...
rule link_raw_data:
    ...
    shell:
        'echo {wildcards.r}; mkdir -p {wildcards.sn}/raw_fastq/; cp {input} {output};'
...
rule raw_reads_composite_reference_bwa_map:
    ...
    shell:
        '(conda info --envs; bwa mem -t {threads} {params.composite_index} '
...
rule run_filtering_of_residual_adapters:
    ...
    shell:
        """
	python3.6 {params.script_path} --input_R1 {input.r1} --input_R2 {input.r2}
	"""
